...
export function setSkeleton(chart) {
  ...
  const factor_margin_body = 1.0;

  // chart.width_svg = chart.width_svg || 750;
  // chart.height_svg = chart.height_svg || 421.875;
  chart.width_svg = chart.width_svg || 750;
  chart.height_svg = chart.height_svg || chart.width_svg * 2 / 3;

  chart.margins = chart.margins || {
    top: chart.height_svg * 0.05,
    bottom: chart.height_svg * 0.05,
    left: chart.width_svg * 0.064,
    right: chart.width_svg * 0.048
  };
  chart.width_g_total =
  ...
  chart.height_g_footer = chart.height_g_total / 4.8;
  chart.height_g_body =
  ...
  chart.y_g_body = chart.height_g_header * 1.4;
...
export function setSkeletonWide(chart) {
  const factor_primary_fontsize = 0.07;
  const factor_secondary_fontsize = 0.045;
  const factor_tertiary_fontsize = 0.035;
  const factor_quaternary_fontsize = 0.029;
